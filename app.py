...
# --- APP UI SETUP ---
st.set_page_config(page_title="Lumber Hub: Column Sync", layout="wide")
...
# --- PROFILE MANAGER ---
existing_profiles = [f.replace(".json", "") for f in os.listdir(".") if f.endswith(".json") and f not in [MILEAGE_CACHE_FILE, MATRIX_FILE]]
if not existing_profiles: existing_profiles = ["Default"]

st.sidebar.header("📁 Profile Manager")
selected_profile = st.sidebar.selectbox("Select Profile", existing_profiles)
new_profile_name = st.sidebar.text_input("OR Create New")
current_profile = new_profile_name if new_profile_name else selected_profile
saved_data = load_json(f"{current_profile}.json")
...
for i in range(1, 7):
    ...
    s = col_s.text_input(f"St {i}", d_states[i-1] if i-1 < len(d_states) else "", key=f"s{i}").upper().strip()
    r = col_r.number_input(f"Rate {i}", value=d_rates[i-1] if i-1 < len(d_rates) else 0.00, key=f"r{i}")
...
saved_cities = saved_data.get("cities_list", "Houston, TX\nChicago, IL")
...
st.sidebar.markdown("---")
st.sidebar.header("📥 Backup & Export")
def create_backup_zip():
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for f in os.listdir("."):
            if f.endswith(".json") and f not in [MILEAGE_CACHE_FILE, MATRIX_FILE]: zf.write(f)
    return buf.getvalue()

st.sidebar.download_button("📦 DOWNLOAD BACKUP (ZIP)", data=create_backup_zip(), file_name=f"LumberHub_Backup.zip", mime="application/zip", use_container_width=True)
...
def get_miles(origin, destination):
    ...
    time.sleep(1.2)
    try:
        headers = {'User-Agent': 'lumber_hub_v95'}
...
def run_calculation(city, df_m, df_s, r_map, r_rule, inc_m, inc_s):
    ...
    for _, r in combined.iterrows():
        ...
        avail = str(r.get('Availability', 'Prompt'))
        ship = str(r.get('Ship Time', 'Prompt'))
...
with tab_pricing:
    ...
    with col_a:
        ...
        df_master = st.data_editor(pd.DataFrame(m_data) if m_data else pd.DataFrame({"Product": [""]*20, "FOB Price": [0.0]*20, "Origin": [""]*20, "Availability": ["Prompt"]*20, "Ship Time": ["Prompt"]*20}), use_container_width=True, num_rows="dynamic", key="m_edit")
    with col_b:
    ...
    if st.sidebar.button("💾 SAVE PROFILE SETTINGS"):
        ...
        save_json(f"{current_profile}.json", config)
        st.sidebar.success("Saved!"); time.sleep(1); st.rerun()

# --- TAB 2: CRM & AUTO-DRAFT ---
with tab_customers:
    st.header("CRM & Quick-Email Shortcut")
    ...
    with col_2:
        ...
        if target_cust != "-- Select --":
            ...
            if email and loc:
                with st.spinner(f"Drafting for {loc}..."):
                    # CRM shortcut now includes all columns
                ...
                if auto_quote:
                    ...
                    st.text_area("Preview Quote Body:", value=auto_quote, height=250)
            else: st.error("Missing Email or Location in CRM.")

    with col_1:
        st.subheader("📝 Master Directory")
        edited = st.data_editor(df_customers, use_container_width=True, num_rows="dynamic", key="crm_main")
